...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:  # 遍历数据集，path为图片路径，im为图片，im0s为原始图片，vid_cap为视频读取对象，s为视频帧率
        ...
        for i, det in enumerate(pred):  # i为索引，det为每个物体的预测框
            ...
            # 保存结果
            save_path = str(save_dir / Path(path[0]).name)
            if vid_path[0] != save_path:
                vid_path[0] = save_path
                if isinstance(vid_writer[0], cv2.VideoWriter):  # 如果vid_writer[i]是cv2.VideoWriter类型
            ...
            #判断是否结束
            if not v2_is_running:
                cv2.destroyAllWindows()  # 关闭所有识别窗口
                return  # 停止当前程序
